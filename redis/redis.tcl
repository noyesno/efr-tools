...
proc ::redis::__dispatch__ {id method args} {
	...
	# Only REDIS commands require a reply callback in non-blocking
	# mode, internal commands don't
	if {$blocking == 0} {
	    if {[llength $args] == 0} {
...
proc ::redis::__method__blocking {id fd { val "" } } {
    if { $val ne "" } {
	set ::redis::blocking($id) [string is true $val]
...
proc ::redis::__method__close {id fd} {
    ...
    catch {unset ::redis::pipelined($id)}
...
proc ::redis::__method__deferred {id fd { val "" } } {
    if { $val ne "" } {
	if { [string is false $val] } {
	    __method__flush $id $fd
	    set blocking $::redis::blocking($id)
	    for { set i 0 } { $i < $::redis::pipelined($id) } { incr i } {
		if { $blocking } {
		    redis_read_reply $fd
		} else {
	...
	set ::redis::deferred($id) [string is true $val]
    }
    return $::redis::deferred($id)
}
